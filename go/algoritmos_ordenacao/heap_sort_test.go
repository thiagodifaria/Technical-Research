...
func TestHeapSort(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(inputCopy, tc.expected) {
				t.Errorf("HeapSort(%v) = %v, want %v", tc.input, inputCopy, tc.expected)
			}
		})
	}
	...
	t.Run("random small", func(t *testing.T) {
		...
		if !reflect.DeepEqual(inputCopy, expected) {
			t.Errorf("HeapSort(random small %v) = %v, want %v", input, inputCopy, expected)
		}
	})
	...
	t.Run("random moderate", func(t *testing.T) {
		...
		HeapSort(inputCopy)
		if !reflect.DeepEqual(inputCopy, expected) {
			t.Errorf("HeapSort(random moderate %v) = %v, want %v", input, inputCopy, expected)
		}
	})
}