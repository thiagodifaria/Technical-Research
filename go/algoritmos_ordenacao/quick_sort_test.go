...
func TestQuickSort(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(inputCopy, tc.expected) {
				t.Errorf("QuickSort(%v) = %v, want %v", tc.input, inputCopy, tc.expected)
			}
		})
	}
	...
	t.Run("random small", func(t *testing.T) {
		...
		if !reflect.DeepEqual(inputCopy, expected) {
			t.Errorf("QuickSort(random small %v) = %v, want %v", input, inputCopy, expected)
		}
	})
	...
	t.Run("random moderate", func(t *testing.T) {
		...
		QuickSort(inputCopy)
		if !reflect.DeepEqual(inputCopy, expected) {
			t.Errorf("QuickSort(random moderate %v) = %v, want %v", input, inputCopy, expected)
		}
	})

	t.Run("random large", func(t *testing.T) {
		r := rand.New(rand.NewSource(time.Now().UnixNano()))
		input := make([]int, 2000)
		for i := range input {
			input[i] = r.Intn(20001) - 10000
		}
		expected := make([]int, len(input))
		copy(expected, input)
		sort.Ints(expected)
		...
		QuickSort(inputCopy)
		if !reflect.DeepEqual(inputCopy, expected) {
			t.Errorf("QuickSort(random large %v) = %v, want %v", input, inputCopy, expected)
		}
	})
}